...
class EVO_generator {
	...
	function reused(){
		$this->lang_array['et'] = eventon_get_event_tax_name_('et', $this->shortcode_args['lang'], $this->evopt1, $this->evopt2); 
		$this->lang_array['et2'] = eventon_get_event_tax_name_('et2', $this->shortcode_args['lang'], $this->evopt1, $this->evopt2); 
	...
	*/
	...
	function shortcode_args_for_cal(){
		...
		$ft_event_priority = (!empty($arg['ft_event_priority']))? 'ft_event_priority="'.$arg['ft_event_priority'].'"': null;
		
		return "<div class='cal_arguments' style='display:none'  hide_past='".$arg['hide_past']."' show_et_ft_img='".$arg['show_et_ft_img']."' event_order='".$arg['event_order']."' {$ft_event_priority} lang='{$arg['lang']}'></div>";
	...
	public function eventon_generate_calendar($args){
		...
		if( $show_upcoming!=1 && ( !empty($this->evopt1['evcal_cal_hide']) && $this->evopt1['evcal_cal_hide']=='no') ||  empty($this->evopt1['evcal_cal_hide'])):		
			...
			// Calendar SHELL
			$content.=$this->get_calendar_top($focused_month_num, $focused_year);
			
			
			// ========================================
			// HEADER with month and year name	- for NONE upcoming list events
			$content.= $this->get_calendar_header($focused_month_num, $focused_year);
	...
	// GET: Calendar top div with variables in attr
	function get_calendar_top($focused_month_num, $focused_year){
		...
		if(has_filter('eventon_cal_class')){
			$addition_class =apply_filters('eventon_cal_class', '');
		}else{
			$addition_class = null;
		}
		...
		// Calendar SHELL
		return "<div id='evcal_calendar_".$cal_id."' class='ajde_evcal_calendar{$addition_class}' cal_ver='".$cal_version."' mapscroll='".$evcal_gmap_scrollw."' mapformat='".$evcal_gmap_format."' mapzoom='".$evcal_gmap_zooml."'  ".$eventcard_open." cur_m='".$focused_month_num."' cur_y='".$focused_year."'>";
	}
	...
	// GET: header section of the calendar only
	function get_calendar_header($focused_month_num, $focused_year){
		$content='';
		$args = $this->shortcode_args;
		...
		// HTML 
		$content.="<div id='evcal_head' class='calendar_header ".$sort_class."' cur_m='".$focused_month_num."' cur_y='".$focused_year."' ev_cnt='".$args['event_count']."' sort_by='sort_date' filters_on='{$this->filters}'>					
			<span id='evcal_prev' class='evcal_arrows evcal_btn_prev' ".$hide_arrows_check."></span>
		...
		// SORT BAR
		$content.= $this->eventon_get_cal_sortbar($args['event_type'], $args['event_type_2']);
	...
	function get_calendar_month_body( $get_new_monthyear, $focus_start_date_range='', $focus_end_date_range=''){
		...
		if($content_li != 'empty'){
			...
			if( $this->is_upcoming_list && !empty($hide_empty_months) && $hide_empty_months=='yes'){
				...
				echo "<div id='evcal_list' class='eventon_events_list'><div class='eventon_list_event'><p class='no_events'>".$this->lang_array['no_event']."</p></div></div>";
			}
		}
	...
	*/	 
	...
	public function generate_event_data(
		...
		foreach($event_list_array as $event):
			...
			*/
			...
			$gmap_trigger = (!empty($ev_vals['evcal_gmap_gen']) && $ev_vals['evcal_gmap_gen'][0]=='yes')? 'gmtrig="1"':'gmtrig="0"';
			...
			<{$html_tag} id='".$unique_id."' class='".$__a_class."' ".$href." ".$target_ex." style='border-left-color: ".$event_color."' ".$gmap_trigger." ".(!empty($gmap_api_status)?$gmap_api_status:null)." ux_val='{$exlink_option}'>{$html_info_line}</{$html_tag}>".$html_event_detail_card."<div class='clear'></div></div>";	
				...
				<div class='eventon_filter evo_sortOpt' filter_field='event_type' filter_val='all' filter_type='tax' style='display:none'>
					...
					<div class='eventon_filter_selection'>
						<p class='filtering_set_val' opts='evs4_in'>"
								.eventon_get_custom_language($this->evopt2, 'evcal_lang)all', 'All')."</p>
	...
	// generate a dynamic ICS file for download for events 
	public function generate_ics_for_event($event_id){
		...
		$event_start_unix = $ev_vals['evcal_srow'][0];
		$event_end_unix = (!empty($ev_vals['evcal_erow']))? $ev_vals['evcal_erow'][0] : $event_start_unix;
		
		
		$name = $the_event->post_title;
		
		
		$location = (!empty($ev_vals['evcal_location']))? $ev_vals['evcal_location'][0] : ''; 
		$start = date_i18n('Ymd', $event_start_unix) . 'T' . date_i18n('His', $event_start_unix) . 'Z';
		$end = date_i18n('Ymd', $event_end_unix) . 'T' . date_i18n('His', $event_end_unix) . 'Z';
		...
		echo "METHOD:REQUEST\n"; // requied by Outlook
		echo "BEGIN:VEVENT\n";
		echo "UID:".date_i18n('Ymd').'T'.date_i18n('His')."-".rand()."-learnphp.co\n"; // required by Outlok
