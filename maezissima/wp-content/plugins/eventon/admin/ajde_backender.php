			...
			foreach($cpav['fields'] as $field){
				...
				switch ($field['type']){
					...
					case 'hiddensection_open':
						...
						$display = (!empty($field['display']) && $field['display']=='none')? 'style="display:none"':null;
						
						$rightside.="<div class='evoSET_hidden_open'><h4>{$field['name']}<span></span>{$legend_code}</h4></div>
						<div class='evoSET_hidden_body' {$display}>";
