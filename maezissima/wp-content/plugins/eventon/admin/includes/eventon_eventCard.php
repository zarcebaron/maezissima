	...
	foreach($array as $box_f=>$box){
		...
		switch($box_f){
				...
				if( $evOPT['evo_morelass']!='yes' && (strlen($object->fulltext) )>600 ){
					$more_code = 
						"<div class='eventon_details_shading_bot'>
							<p class='eventon_shad_p' content='less'><span class='ev_more_text' txt='".eventon_get_custom_language($evOPT, 'evcal_lang_less','less')."'>".eventon_get_custom_language($evoOPT2, 'evcal_lang_more','more')."</span><span class='ev_more_arrow'></span></p>
						</div>";
			...
			case 'learnmoreICS':
				...
				if( !empty($object->learnmorelink) && !empty($evOPT['evo_ics']) && $evOPT['evo_ics']=='yes'){
					...
					<div class='tb'>
						...
						<a class='evcal_col50 dark1 evo_clik_row' href='".AJDE_EVCAL_URL."/ics.php?event_id=".$object->event_id."'>
							<span class='evcal_evdata_row ' >
				...
				// only learn more
					...
					$OT.="<div class='evorow bordb'>
						<a class='evcal_evdata_row evo_clik_row dark1' href='".AJDE_EVCAL_URL."/ics.php?event_id=".$object->event_id."'>
							<span class='evcal_evdata_icons'><i class='fa ".get_eventON_icon('evcal__fai_008', 'fa-calendar-o',$evOPT )."'></i></span>													
							<h3 class='evo_h3'>".eventon_get_custom_language($evoOPT2, 'evcal_evcard_addics','Add to your calendar')."</h3>
						</a>
						".$end."</div>";
			...
			case 'paypal':
				...
				$OT.="<div class='evorow evcal_evdata_row bordb'>
