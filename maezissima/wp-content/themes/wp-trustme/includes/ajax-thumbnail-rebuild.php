...
class ct_AjaxThumbnailRebuild {

	function ct_AjaxThumbnailRebuild() {
		add_action( 'admin_menu', array(&$this, 'addAdminMenu') );
	}

	function addAdminMenu() {
		add_theme_page( __( 'Rebuild all Thumbnails', 'color-theme-framework' ), __( 'Rebuild Thumbnails', 'color-theme-framework'
 ), 'edit_theme_options', 'ajax-thumbnail-rebuild', array(&$this, 'ManagementPage') );
	...
	function ManagementPage() {
		...
		function regenerate() {
			...
			setMessage("<p><?php _e('Reading attachments...', 'color-theme-framework') ?></p>");
			...
			jQuery.ajax({
				...
				success: function(result) {
					...
					if (!list) {
						setMessage("<?php _e('No attachments found.', 'color-theme-framework')?>");
					...
					function regenItem() {
						if (curr >= list.length) {
							jQuery("#ajax_thumbnail_rebuild").removeAttr("disabled");
							setMessage("<?php _e('Done.', 'color-theme-framework') ?>");
							return;
						}
				...
				error: function(request, status, error) {
					setMessage("<?php _e('Error', 'color-theme-framework') ?>" + request.status);
				}
			});
		}
		...
		<form method="post" action="" style="display:inline; float:left; padding-right:30px;">
		    <h4><?php _e('Select which thumbnails you want to rebuild', 'color-theme-framework'); ?>:</h4>
			<a href="javascript:void(0);" id="size-toggle"><?php _e('Toggle all', 'color-theme-framework'); ?></a>
			<div id="sizeselect">
			<?php
			foreach ( ct_ajax_thumbnail_rebuild_get_sizes() as $s ):
			?>
				...
				<label>
					...
					<?php if ($s['crop']) _e('cropped', 'color-theme-framework'); ?>)
				</label>
				<br/>
			<?php endforeach;?>
			</div>
			<p>
				...
				<label><?php _e('Only rebuild featured images', 'color-theme-framework'); ?></label>
			</p>

			<p><?php _e("Note: If you've changed the dimensions of your thumbnails, existing thumbnail images will not be deleted.",
			'color-theme-framework'); ?></p>
			<input type="button" onClick="javascript:regenerate();" class="button"
			       name="ajax_thumbnail_rebuild" id="ajax_thumbnail_rebuild"
			       value="<?php _e( 'Rebuild All Thumbnails', 'color-theme-framework' ) ?>" />
			<br />
		</form>

		<div id="thumb" style="display:none;"><h4><?php _e('Last image', 'color-theme-framework'); ?>:</h4><img id="thumb-img" /></div>
...
function ct_ajax_thumbnail_rebuild_ajax() {
	global $wpdb;
	...
	if ($action == "getlist") {
		...
		if ( FALSE !== $fullsizepath && @file_exists($fullsizepath) ) {
			set_time_limit( 30 );
			wp_update_attachment_metadata( $id, ct_wp_generate_attachment_metadata_custom( $id, $fullsizepath, $thumbnails ) );
		}

		die( wp_get_attachment_thumb_url( $id ));
	}
}
add_action('wp_ajax_ajax_thumbnail_rebuild', 'ct_ajax_thumbnail_rebuild_ajax');

//add_action( 'plugins_loaded', create_function( '', 'global $AjaxThumbnailRebuild; $AjaxThumbnailRebuild = new AjaxThumbnailRebuild();' ) );

global $ct_AjaxThumbnailRebuild; $ct_AjaxThumbnailRebuild = new ct_AjaxThumbnailRebuild();

function ct_ajax_thumbnail_rebuild_get_sizes() {
 ...
 */
function ct_wp_generate_attachment_metadata_custom( $attachment_id, $file, $thumbnails = NULL ) {
	...
	if ( preg_match('!^image/!', get_post_mime_type( $attachment )) && file_is_displayable_image($file) ) {
		...
		$sizes = ct_ajax_thumbnail_rebuild_get_sizes();
